...
| ファイル            | 提供するもの                                                    |
| ------------------- | --------------------------------------------------------------- |
| `_palette.scss`     | カラースケール生成（`make-scale()`, `emit-scale()` 等）         |
...
:root {
  // 名前付きスケールを展開（--blue-1 .. --blue-12）
  @include palette.emit-scale("blue");

  // 任意の色（ブランドカラー等）から12段階スケールを生成
  @include palette.emit-scale("brand", palette.make-scale(#0078d4));
}
```

生成されるCSS変数（ビルド時に確定した具体値のみ）:

```css
:root {
  --blue-1: rgb(239, 246, 255); /* 最も明るい */
  --blue-7: rgb(21, 93, 252); /* solid 相当 */
  --blue-12: rgb(5, 12, 46); /* 最も暗い */
  --brand-7: rgb(0, 120, 212); /* ブランドカラーそのもの */
  /* ... */
}
```
