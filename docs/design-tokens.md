...
各名前付きスケール（`--{family}-{1..12}`）に共通するステップ。数値は強調度・輝度の相対的な位置を示す。
実装は `src/utilities/_color-scales.scss`（OKLCH チャンネル値の生データ）と
`src/utilities/_palette.scss`（ビルド時の sRGB 変換・スケール生成関数）。

```
1  → 最も淡い（背景tint用・最明）
...
7  → 中間（filled ボタン等のソリッドカラー）
...
12 → 最も濃い（テキスト用）
```

ステップ 1〜11 は Tailwind CSS v4 の 50〜950 に対応し、ステップ 12 はビルド時に外挿で生成される。
明度・彩度カーブは OKLCH 空間でファミリーごとに知覚調整されているため、
HSL の固定明度リストのような「色相によって見え方が変わる」問題は起きない。
任意の色からスケールを起こす場合は `palette.make-scale($color)` を使う。
